function initMenuItems(app, callback) {
  ...
  sql.push('INNER JOIN software_version_menu_item svmi ON svmi.menu_item_id = mi.menu_item_id AND svmi.deleted_at IS NULL');
  sql.push('AND svmi.software_version_id = $1');
  sql.push('INNER JOIN software_version sv ON sv.software_version_id = svmi.software_version_id AND sv.deleted_at IS NULL');
  sql.push('WHERE mi.deleted_at IS NULL');
