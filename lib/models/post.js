...
function Post() {
  ...
  this._validationRules = [
    { field: 'title', rule: 'NotEmpty', message: 'You must add a title' },
...
Post.prototype.setPostData = function (postData) {
...
Post.prototype.save = function (callback) {
  this._postData.save(this._data, callback);
};

Post.prototype.update = function (callback) {
  this._postData.update(this._data, callback);
};

Post.prototype.findById = function (id, callback) {
  this._postData.findById(id, callback);
};

Post.prototype.find = function (filters, callback) {
  this._postData.find(filters, callback);
};

Post.prototype.getLatest = function (limit, callback) {
  //this._postData.getLatest(limit, callback);
  this._postData.getLatest(limit, function (err, posts) {
    if (err) {
      callback(err);
    }
    else {
      ...
      for (p = 0; p < posts.length; p++) {
        (function (p) {
          this._postData.getTags(posts[p].post_id, function (err, tags) {
            if (error) {
              return;
            }
            else if (err) {
...
Post.prototype.getUnapproved = function (callback) {
  this._postData.getUnapproved(callback);
};

Post.prototype.getBySlug = function (slug, callback) {
  this._postData.getBySlug(slug, function (err, post) {
    if (err) {
      callback(err);
    }
    else {
      this._postData.getTags(post.post_id, function (err, tags) {
        if (err) {
          callback(err);
        }
        else {
...
Post.prototype.hasChanged = function (callback) {
  this._postData.findById(this._data.post_id, function (err, post) {
    var changed = false;
...
Post.prototype.approve = function (callback) {
  this._postData.approve(this._data, function (err) {
    if (err) {
      callback(err);
    }
    else {
      this.findById(this._data.post_id, function (err, postData) {
        if (err) {
          callback(err);
        }
        else {
...
Post.prototype.reject = function (callback) {
  this._postData.reject(this._data, callback);
};

Post.prototype.delete = function (callback) {
  this._postData.delete(this._data, callback);
};

Post.prototype.getTagByName = function (name, callback) {
  this._postData.getTagByName(name, callback);
};

Post.prototype.getPostsByTagId = function (callback) {
  this._postData.getPostsByTagId(this._data, callback);
};

Post.prototype.getActivity = function (callback) {
  this._postData.getActivity(callback);
}
  ...
  if (hashtags === null) {
    ...
    tag.saveMultiple(function (err, tag_ids) {
      if (err || !tag_ids) {
        ...
        this._postData._savePostTags({post_id: this._data.post_id, tag_ids: tag_ids, by: this._data.by}, callback);
      }
    }.bind(this));
  }
};
