...
Post.prototype.findById = function (post_id, callback) {
  this._postData.find({ post_id: post_id }, callback);
};
...
Post.prototype.getLatest = function (limit, callback) {
  ...
  this._postData.find(data, function (err, posts) {
    if (err) {
      callback(err);
    }
    else {
...
Post.prototype.getBySlug = function (slug, callback) {
  this._postData.find({ slug: slug, post_status_type_id: 3 }, function (err, post) {
    if (err) {
      callback(err);
    }
    else {
...
Post.prototype.hasChanged = function (callback) {
  this._postData.find({ post_id: this._data.post_id }, function (err, post) {
    var changed = false;
