...
TagStore.prototype.find = function (filters, callback) {
  ...
  sql.push('t.inserted_at, t.inserted_by,');
  sql.push('u.name user_name');
  sql.push('FROM tag t');
  sql.push('LEFT JOIN "user" u ON u.user_id = t.inserted_by');
  sql.push('LEFT JOIN post_tag pt ON pt.tag_id = t.tag_id AND pt.deleted_at IS NULL');
  sql.push('LEFT JOIN published_post p ON p.post_id = pt.post_id');
  sql.push('WHERE t.deleted_at IS NULL');
...
TagStore.prototype._insertTag = function (data, callback) {
  ...
  sql.push('(name, pretty_name, content, inserted_at, inserted_by)');
  sql.push('VALUES(');
