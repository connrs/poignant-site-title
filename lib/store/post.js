...
PostStore.prototype.getUnapproved = function (callback) {
  this._pg.client(function (err, query) {
    ...
    sql.push('SELECT p.post_id, pst.post_status_type_id, p.title, p.summary, p.content, p.published, p.inserted_at, p.updated_at, p.slug,');
    ...
    sql.push('COALESCE(EXTRACT(EPOCH FROM p.updated_at),EXTRACT(EPOCH FROM p.published)) last_modified,');
    ...
    sql.push('INNER JOIN post_status ps ON ps.post_id = p.post_id AND ps.deleted_at IS NULL AND ps.post_status_type_id IN(2,5)');
    sql.push('LEFT JOIN (');
    sql.push('SELECT p.parent_post_id, p.post_id');
    sql.push('FROM post p');
    sql.push('INNER JOIN post_status ps ON ps.post_id = p.post_id AND ps.deleted_at IS NULL AND ps.post_status_type_id = 5');
    sql.push('WHERE p.parent_post_id IS NOT NULL AND p.deleted_at IS NULL');
    ...
    sql.push('LEFT JOIN provider_user pu ON pu.user_id = u.user_id AND pu.deleted_at IS NULL');
    sql.push('LEFT JOIN provider prov ON prov.provider_id = pu.provider_id AND prov.deleted_at IS NULL');
    sql.push('WHERE p.deleted_at IS NULL');
    sql.push('ORDER BY p.inserted_at DESC');
...
PostStore.prototype.update = function (data, callback) {
  this._pg.transaction(function (err, client) {
    ...
    sql.push('(parent_post_id, title, slug, summary, content, location, published, inserted_at, inserted_by)');
    sql.push('SELECT ');
    ...
    client.query(sql.join('\n'), params, function (err, results, client) {
      ...
      sql.push('(post_id, post_status_type_id, inserted_at, inserted_by)');
...
PostStore.prototype._insertPost = function (client, data, callback) {
  ...
  sql.push('INSERT INTO post')
  sql.push('(title, slug, summary, content, published, location, inserted_at, inserted_by)');
  sql.push('VALUES(');
...
PostStore.prototype._updateStatus = function (client, data, callback) {
  ...
  query.push('WHERE post_id = $3 AND deleted_at IS NULL;');
  ...
  client.query(query.join('\n'), params, function (err, result, client) {
    ...
    if (err) {
      ...
      query.push('(post_id, post_status_type_id, inserted_at, inserted_by)');
  ...
  if (tagCount === 0) {
    ...
    data.tag_ids.forEach(function (tag_id) {
      ...
      if (!insertError) {
        ...
        query.push('(post_id, tag_id, inserted_at, inserted_by)');
        query.push('VALUES($1, $2, NOW(), $3)');
...
PostStore.prototype.getTags = function (post_id, callback) {
  ...
  sql.push('INNER JOIN tag t ON t.tag_id = pt.tag_id AND t.deleted_at IS NULL');

  sql.push('WHERE pt.post_id = $' + ++p);
  params.push(post_id);

  sql.push('AND pt.deleted_at IS NULL');
...
PostStore.prototype._find = function (filters, callback) {
  this._pg.client(function (err, query) {
    ...
    sql.push('SELECT p.post_id, p.parent_post_id, pst.post_status_type_id, p.title, p.summary, p.content, p.published, p.inserted_at, p.updated_at, p.slug, p.inserted_by,');
    ...
    sql.push('u.name author_name, u.url author_url');
    sql.push('FROM post p');
    sql.push('INNER JOIN post_status ps ON ps.post_id = p.post_id AND ps.deleted_at IS NULL');
    ...
    if (filters.tag_id) {
      sql.push('INNER JOIN post_tag pt ON pt.post_id = p.post_id AND pt.deleted_at IS NULL AND pt.tag_id = $' + ++p);
      sql.push('INNER JOIN tag t ON t.tag_id = pt.tag_id AND t.deleted_at IS NULL');
...
PostStore.prototype._count = function (filters, callback) {
  this._pg.client(function (err, query) {
    ...
    if (filters.tag_id) {
      sql.push('INNER JOIN post_tag pt ON pt.post_id = p.post_id AND pt.deleted_at IS NULL');
      sql.push('INNER JOIN tag t ON t.tag_id = pt.tag_id AND t.deleted_at IS NULL AND t.tag_id = $' + ++p);
      params.push(+filters.tag_id);
    }
