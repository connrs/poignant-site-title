...
CommentStore.prototype._count = function (data, callback) {
  this._pg.client(function (err, query) {
    if (err) {
      ...
      sql.push('INNER JOIN comment_status cs ON cs.comment_id = c.comment_id AND cs.deleted_at IS NULL');
      ...
      if (data.post_id) {
        sql.push('INNER JOIN comment_post cp ON cp.comment_id = c.comment_id AND cp.deleted_at IS NULL AND cp.post_id = $' + ++p);
        params.push(data.post_id);
      }

      sql.push('LEFT JOIN "user" u ON u.user_id = c.inserted_by AND u.deleted_at IS NULL');

      sql.push('WHERE c.deleted_at IS NULL');
...
CommentStore.prototype._find = function (data, callback) {
  this._pg.client(function (err, query) {
    if (err) {
      ...
      sql.push('SELECT c.comment_id, c.content, c.inserted_at,');
      ...
      sql.push('ORDER BY c.inserted_at ASC');
...
CommentStore.prototype._insertComment = function (data, callback) {
  this._pg.client(function (err, query) {
    if (err) {
      ...
      sql.push('(comment_type_id, content, inserted_at, inserted_by)');
      sql.push('VALUES(');
...
CommentStore.prototype._insertStatus = function (data, callback) {
  this._pg.client(function (err, query) {
    if (err) {
      ...
      sql.push('(comment_id, comment_status_type_id, inserted_at, inserted_by)');
      sql.push('VALUES('); 
...
CommentStore.prototype._updateStatus = function (data, callback) {
  this._pg.transaction(function (err, client) {
    if (err) {
      ...
      query.push('WHERE comment_id = $3 AND deleted_at IS NULL;');
      ...
      client.query(query.join('\n'), params, function (err, result, client) {
        ...
        if (err) {
          ...
          query.push('(comment_id, comment_status_type_id, inserted_at, inserted_by)');
...
CommentStore.prototype._insertCommentPost = function (data, callback) {
  this._pg.client(function (err, query) {
    if (err) {
      ...
      sql.push('(comment_id, post_id, inserted_at, inserted_by)');
      sql.push('VALUES(');
